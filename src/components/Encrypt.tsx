...
export default function Encrypt() {
  ...
  async function encrypt() {
    try{
      const res: {error: boolean, message: string} = await invoke("encrypt", {filePaths, password, confirmPassword})
  ...
  return (
    <div className="w-full flex flex-col gap-5">
      <button className="btn hover:text-white hover:bg-gray-700 bg-gray-500 text-black hover:border-white" onClick={async () => await chooseFiles()}>Pick Files</button>
      ...
      {filePaths.length > 0 &&
        <form onSubmit={e=>e.preventDefault()} className="flex flex-col gap-3">
          ...
          <div className="flex flex-col">
            <label htmlFor="confirm-password" className="font-bold mr-2">Confirm Password:</label>
            <input required onChange={e => setConfirmPassword(e.target.value)} value={confirmPassword} id="confirm-password" name="confirm-password" type="password" className="input input-bordered" />
          </div>
          {error && <h2 className="text-red-500 font-bold">{error}</h2>}
          {res && <h2 className="text-green-500 font-bold">{res}</h2>}
          <div>
            <button onClick={async () => await encrypt()} className="w-full btn hover:border-gray-500">Encrypt</button>
          </div>
        </form>}
